// Paper parity (brief Part B, step 12): blank, printable renderings of the
// formulation interview and the ABC/episode form — same content and same
// option pools as the on-screen forms (fbaContent.ts, escalationContent.ts),
...
import { ESCALATION_ITEMS, ESCALATION_PHASES, ESCALATION_PHASE_LABEL } from './escalationContent'
import { ANTECEDENT_CONTEXT_OPTIONS, CONSEQUENCE_OPTIONS, SETTING_EVENT_OPTIONS } from './fbaContent'
import { ANTECEDENT_TAGS, CONSEQUENCE_TAGS, FREQUENCY_SCALE, RISK_FLAG_OPTIONS, SEVERITY_SCALE } from './scales'
...
function checklist(items: { id: string; label: string }[]): string {
  return `<div class="checklist">${items
    .map((i) => `<span class="item"><span class="checkbox"></span>${escapeHtml(i.label)}</span>`)
    .join('')}</div>`
}
...
<body>
  ...
  <p class="meta">Blank template — print and fill in by hand, or use the on-screen form instead. Same options either way.</p>
  ${body}
</body>
</html>`
}
...
export function renderBlankFormulationForm(): string {
  const body = `
    ...
    <h2>Antecedents / context that apply to this behaviour</h2>
    ${checklist(ANTECEDENT_CONTEXT_OPTIONS)}
    <p>Other (write in):</p>
    ${blankLines(2)}

    <h2>Consequences that apply to this behaviour</h2>
    ${checklist(CONSEQUENCE_OPTIONS)}
    <p>Other (write in):</p>
    ${blankLines(2)}

    <h2>Setting events that apply to this behaviour</h2>
    ${checklist(SETTING_EVENT_OPTIONS)}
    <p>Other (write in):</p>
    ${blankLines(2)}

    <h2>Escalation cycle</h2>
    ${ESCALATION_PHASES.map(
      (phase) => `
      <div class="phase-block">
        <h3>${escapeHtml(ESCALATION_PHASE_LABEL[phase])}</h3>
        ${checklist(ESCALATION_ITEMS[phase])}
        <p>Other (write in):</p>
        ${blankLines(1)}
      </div>`,
    ).join('')}
...
export function renderBlankAbcForm(): string {
  const body = `
    ...
    <p>Describe:</p>
    ${blankLines(3)}
    <p>Antecedent category (circle one):</p>
    ${checklist(ANTECEDENT_TAGS.map((t) => ({ id: t.value, label: t.label })))}

    <h2>Consequence — what happened immediately after</h2>
    ${checklist(CONSEQUENCE_OPTIONS)}
    <p>Describe:</p>
    ${blankLines(3)}
    <p>Consequence category (circle one):</p>
    ${checklist(CONSEQUENCE_TAGS.map((t) => ({ id: t.value, label: t.label })))}

    <h2>Severity (practical rating, not a validated measure)</h2>
    ${checklist(SEVERITY_SCALE.map((s) => ({ id: String(s.value), label: `${s.value} — ${s.label}` })))}

    <h2>Frequency / context rating</h2>
    ${checklist(FREQUENCY_SCALE.map((f) => ({ id: String(f.value), label: `${f.value} — ${f.label}` })))}

    <h2>Risk flags</h2>
    ${checklist(RISK_FLAG_OPTIONS.map((r) => ({ id: r.value, label: r.label })))}
  `
